...
`parity-algo-geometry-stack` is centered on algorithms work in OCaml. The technical target is: Package an OCaml local lab for geometry analysis with round-trip fixtures, lossless normalization checks, and documented operating limits.
...
The score is deterministic. The current threshold is 176. Risk has a penalty of 6, latency has a penalty of 2, and weight contributes a bonus of 3.
